...
@user_registration_router.message(User_MainStates.before_registration)
async def process_action(message: Message, state: FSMContext, session: AsyncSession) -> None:
    """Начинает процесс регистрации."""
    logger.info(f"Пользователь {message.from_user.id} начал верификацию")
    user_code = message.text.strip().upper()
    chat_id = message.chat.id
    username = message.from_user.username

    code_exists, participant_id = await check_user_code(session, user_code)
    print(code_exists)
    if not code_exists:
        await message.answer(
            "❌ *Код не найден!*\n"
            "Проверьте правильность кода или попробуйте получить новый на сайте.",
            parse_mode="Markdown"
        )
        return

    success = await verify_user_code(
        session,
        code=user_code,
    ...
    if success:
        await message.answer(
            ...
            f"Ваш ID участника: `{participant_id}`\n\n"
            "Теперь вы можете пользоваться всеми функциями бота.",
            parse_mode="Markdown",
            reply_markup=reply.menu_kb
        )
        await state.set_state(User_MainStates.after_registration)  # Сбрасываем состояние
    else:
        await message.answer(
            "⚠️ *Ошибка верификации!*\n"
